...
class MTAT_Dataset(Dataset):
    ...
    def audio_load(self, path):
        audio = np.load(os.path.join(self.data_path, "mtat", "npy", path.replace(".mp3",".npy")), mmap_mode='r')
    ...
    def get_train_item(self, index):
        ...
        audio_tensor = self.audio_load(item['path'])
        return {
            "audio":audio_tensor, 
            "binary":binary
            }
    ...
    def get_eval_item(self, index):
        ...
        audio = np.load(os.path.join(self.data_path, "mtat", "npy", audio_path.replace(".mp3",".npy")), mmap_mode='r')
