...
class MTG_Dataset(Dataset):
    ...
    def audio_load(self, track_id):
        audio = np.load(os.path.join(self.data_path, "mtg", "npy", track_id.replace(".mp3",".npy")), mmap_mode='r')
    ...
    def get_eval_item(self, index):
        ...
        path = item['path']
        audio = np.load(os.path.join(self.data_path, "mtg", "npy", path.replace(".mp3",".npy")), mmap_mode='r')
